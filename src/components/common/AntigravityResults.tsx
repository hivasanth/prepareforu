...
export const ScoreCard: React.FC<{ score: number; total?: number; label?: string; subtitle?: string; children?: React.ReactNode; className?: string }> = ({ score, total, label = 'Final Score', subtitle, children, className = '' }) => {
  return (
    <Card className={`flex flex-col items-center text-center p-6 md:p-8 lg:p-12 relative overflow-hidden ${className}`}>
      <div className="absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl -mr-32 -mt-32 pointer-events-none bg-primary/5" />
      ...
      <H3 className="mb-0.5 md:mb-1 text-[16px] md:text-[20px] font-black">Assessment Finalized</H3>
      {subtitle && <p className="text-[11px] md:text-[13px] text-text-secondary opacity-50 max-w-md mx-auto mb-4 md:mb-5 leading-relaxed font-bold">{subtitle}</p>}
      {children}
    </Card>
  )
}
...
export const CTACard: React.FC<{ title: string; subtitle: string; icon: LucideIcon; onClick: () => void; badge?: string; className?: string }> = ({ title, subtitle, icon: Icon, onClick, badge, className = '' }) => {
  return (
    ...
    >
      <div className="absolute inset-0 transition-transform duration-500 group-hover:scale-110 bg-gradient-to-r from-primary via-primary-hover to-primary" />
      <div className="relative rounded-[18px] md:rounded-[24px] p-4 md:p-6 lg:p-7 flex flex-col sm:flex-row items-center justify-between gap-4 transition-colors group-hover:bg-transparent bg-card-bg">
        <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
          {badge && <Label className="mb-1.5 uppercase tracking-widest text-[9px] text-primary group-hover:text-white/60">{badge}</Label>}
          <H3 className="transition-colors text-[16px] md:text-[18px] font-black group-hover:text-white">{title}</H3>
          {subtitle && <Body className="transition-colors mt-0.5 text-[12px] md:text-[13px] group-hover:text-white/80">{subtitle}</Body>}
        </div>
        <div className="w-10 h-10 md:w-11 md:h-11 rounded-lg md:rounded-xl flex items-center justify-center transition-all group-hover:translate-x-2 shrink-0 bg-primary/10 text-primary group-hover:bg-white/20 group-hover:text-white">
          <Icon size={20} />
        </div>
      </div>
    </button>
