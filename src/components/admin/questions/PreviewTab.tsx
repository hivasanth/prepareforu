...
export function PreviewTab({ parsedData, validationSummary, duplicateCount }: PreviewTabProps) {
  ...
  return (
    <div className="space-y-6 animate-in zoom-in-95 duration-500">
      {validationSummary && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={stat.label} className={`p-4 rounded-2xl border text-center ${!isDark ? 'bg-white/50 border-primary/10' : 'bg-hover-bg/20 border-border-subtle/30'}`}>
      ...
      {duplicateCount > 0 && (
        <div role="alert" className={`p-4 rounded-2xl border flex items-center gap-3 ${!isDark ? 'bg-amber-50/80 border-amber-200' : 'bg-amber-500/5 border-amber-500/20'}`}>
      ...
      <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
        {parsedData.map((item, i) => (
          <div key={i} className={`p-4 rounded-2xl border transition-all ${!isDark ? 'bg-white/40 border-primary/10 hover:border-primary/30' : 'bg-hover-bg/20 border-border-subtle/30 hover:border-primary/30'}`}>
