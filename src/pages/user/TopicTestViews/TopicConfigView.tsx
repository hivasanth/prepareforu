...
import { Card, IconButton, Stack, Button } from '../../../components/common/AntigravityUI'
...
export function TopicConfigView({
  selectedSubject,
  subjectCounts,
  questionCount,
  setQuestionCount,
  ...
  onLaunch,
  onBack
}: TopicConfigViewProps) {
  const { isDark } = useTheme();

  const totalQuestions = subjectCounts[selectedSubject] || 0;
  let options = [10, 20, 30].filter(cnt => cnt <= totalQuestions);
  if (totalQuestions > 0 && !options.includes(totalQuestions) && totalQuestions < 30) {
  ...
  useEffect(() => {
    if (totalQuestions > 0 && questionCount > totalQuestions) {
      ...
      if (valid.length > 0) setQuestionCount(valid[0]);
    }
  }, [totalQuestions, questionCount, setQuestionCount]);
      ...
      <Card className="p-8 lg:p-12 space-y-8 shadow-xl border-primary/10">
        <div className="flex items-center gap-4 border-b border-border-subtle pb-6">
          ...
          <div className="flex flex-col">
            <h2 className="text-[24px] font-black text-text-primary uppercase tracking-tight m-0">{selectedSubject}</h2>
        ...
        <Stack gap={32}>
          ...
          <div className="p-6 rounded-[20px] bg-hover-bg/30 border border-border-subtle space-y-4">
            <div className="flex items-start gap-4">
              <div className={`w-8 h-8 flex items-center justify-center flex-shrink-0 ${!isDark ? 'ancient-icon-badge text-[#C8960C]' : 'rounded-full bg-primary/10 text-primary'}`}>
                <Timer size={16} />
              </div>
            ...
            <div className="flex items-start gap-4">
              <div className={`w-8 h-8 flex items-center justify-center flex-shrink-0 ${!isDark ? 'ancient-icon-badge text-[#C8960C]' : 'rounded-full bg-success/10 text-success'}`}>
                <Target size={16} />
              </div>
              <div className="flex flex-col pt-1">
