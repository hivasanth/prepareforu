...
export const ReviewLayout: FC<ReviewLayoutProps> = ({
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-app-bg' : 'light'}`}>
      <FixedBackButton onClick={onBack} />

      <div className="max-w-[1200px] mx-auto px-4 py-6 md:py-10 space-y-8">
        <div className={`${isDark ? 'bg-card-bg border border-border-subtle shadow-2xl' : 'ancient-card'} p-8 md:p-10 text-center space-y-6 relative overflow-hidden rounded-[32px]`}>
          <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-primary via-secondary to-primary" />
          <IconBadge icon={Trophy} size="6xl" shape="circle" className={`mx-auto mb-4 ${isDark ? '' : 'ancient-icon-badge'}`} darkClassName="rounded-full bg-primary/10 text-primary" />
          <div>
          ...
          <div className={`grid grid-cols-2 sm:grid-cols-4 gap-4 py-6 ${isDark ? 'border-y border-border-subtle' : ''}`}>
            <StatCard icon={Target} label="Accuracy" value={`${stats.accuracy}%`} color="#7C3AED" />
            <StatCard icon={CheckCircle2} label="Correct" value={stats.correct} color="#16A34A" />
            <StatCard icon={XCircle} label="Wrong" value={stats.wrong} color="#DC2626" />
            <StatCard icon={Clock} label="Time" value={`${Math.floor(stats.timeTaken / 60)}m`} color="#0891B2" />
          </div>
        ...
        <div className={`${isDark ? 'bg-card-bg border border-border-subtle shadow-xl' : 'ancient-card'} p-6 md:p-8 lg:p-12 space-y-8 rounded-[32px]`}>
          <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b ${isDark ? 'border-border-subtle' : 'border-[#A87828]/30'}`}>
          ...
          <div className="relative group">
            ...
            <input
              ...
              className={`w-full pl-14 pr-6 py-4 rounded-[20px] border-2 outline-none font-bold transition-all shadow-sm ${
                isDark
                  ? 'bg-app-bg border-border-subtle focus:border-primary focus:ring-4 focus:ring-primary/5 text-text-primary placeholder:text-text-secondary/30'
                  : 'bg-[#F5EAD4] border-[#A87828]/40 focus:border-[#A87828] text-[#3D1F08] placeholder:text-[#7A5520]/50'
              }`}
            />
          </div>
          ...
          {(!children || (Array.isArray(children) && children.length === 0)) && (
            <div className={`py-24 text-center border-2 border-dashed rounded-[32px] ${
              isDark ? 'border-border-subtle' : 'border-[#A87828]/30'
            }`}>
